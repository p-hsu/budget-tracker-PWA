...
function checkDatabase() {
  ...
  // Open a transaction on your pendingTransactions db
  let transaction = db.transaction(['pendingTransactions'], 'readwrite');

  // access your pendingTransactions object
    ...
    if (getAll.result.length > 0) {
      fetch('/api/transaction/bulk', {
        ...
        .then((res) => {
          ...
          if (res.length !== 0) {
            // Open another transaction to pendingTransactions with the ability to read and write
            transaction = db.transaction(['pendingTransactions'], 'readwrite');

            // Assign the current store to a variable
            const currentStore = transaction.objectStore('pendingTransactions');
